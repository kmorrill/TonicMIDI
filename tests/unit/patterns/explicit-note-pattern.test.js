...
describe("ExplicitNotePattern", () => {
  ...
  it("supports multiple notes per step (chords) with individual durations", () => {
    const pattern = new ExplicitNotePattern([
      ...
      [
        { note: "C4", durationStepsOrBeats: 2 },
        { note: "E4", durationStepsOrBeats: 3 },
        { note: "G4", durationStepsOrBeats: 1 }
      ],
      // Step 1: Single note
      "D4",
      ...
      [
        { note: "F4", velocity: 90, durationStepsOrBeats: 2 },
        { note: "A4", velocity: 80, durationStepsOrBeats: 2 },
        { note: "C5", velocity: 70, durationStepsOrBeats: 2 }
      ]
    ]);
    ...
    expect(chordC[0].durationStepsOrBeats).toBe(2);
    expect(chordC[1].note).toBe("E4");
    expect(chordC[1].durationStepsOrBeats).toBe(3);
    expect(chordC[2].note).toBe("G4");
    expect(chordC[2].durationStepsOrBeats).toBe(1);
    ...
    expect(noteD[0].durationStepsOrBeats).toBe(1);
  ...
  it("handles string arrays as multiple notes per step with default durations", () => {
    ...
    expect(chordC).toHaveLength(3);
    expect(chordC[0].note).toBe("C4");
    expect(chordC[0].durationStepsOrBeats).toBe(1);
    expect(chordC[1].note).toBe("E4");
    expect(chordC[1].durationStepsOrBeats).toBe(1);
    expect(chordC[2].note).toBe("G4");
    expect(chordC[2].durationStepsOrBeats).toBe(1);
